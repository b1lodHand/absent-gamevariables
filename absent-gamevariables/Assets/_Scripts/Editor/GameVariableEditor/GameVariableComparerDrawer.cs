...
[CustomPropertyDrawer(typeof(GameVariableComparer), true)]
public class GameVariableComparerDrawer : PropertyDrawer
{
    ...
    private VisualElement DrawGUI(SerializedProperty property)
    {
        ...
        var comparisonProp = property.FindPropertyRelative("CompareBy");
        ...
        container.styleSheets.Add(AssetDatabase.LoadAssetAtPath<StyleSheet>("Assets/Editor/GameVariableEditor/GameVariableComparer.uss"));
        ...
        // instantiate selector for variable comparison.
        EnumField comparisonSelector = new EnumField();
        comparisonSelector.BindProperty(comparisonProp);
        comparisonSelector.AddToClassList("compSelector");
        ...
        container.Add(comparisonSelector);
        ...
        void RefreshCompSelector()
        {
            ...
            comparisonSelector.value = GameVariableComparer.ComparisonType.EqualsTo;
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        var comparisonSelection = property.FindPropertyRelative("CompareBy");
        ...
        var comparisonTypeSelectorWidth = 30f;
        ...
        var comparisonTypeSelectorRect = new Rect(horizontalPointer, position.y, comparisonTypeSelectorWidth, position.height);

        horizontalPointer += comparisonTypeSelectorWidth;
        horizontalPointer += horizontalSpace;
        ...
        //if (hasBankSelector) bankSelector.SetEnumValue((VariableBankOption)EditorGUI.
        ...
        // force setting comparison type to equals if variable type is bool. (and also disabling the editor)
        if (targetBank != null && (targetBank.HasBoolean(targetVariableName) || 
                                   targetBank.HasString(targetVariableName) ||
                                   targetVariableName == GameVariableBank.Null))
        {
            GUI.enabled = false;
            comparisonSelection.enumValueIndex = (int)(GameVariableComparer.ComparisonType.EqualsTo);
            EditorGUI.EnumPopup(comparisonTypeSelectorRect, (GameVariableComparer.ComparisonType)comparisonSelection.enumValueIndex);
        }

        // drawing the comparison type selector.
        else
        {
            comparisonSelection.enumValueIndex = (int)((GameVariableComparer.ComparisonType)EditorGUI.
                EnumPopup(comparisonTypeSelectorRect, (GameVariableComparer.ComparisonType)comparisonSelection.enumValueIndex));
        }
